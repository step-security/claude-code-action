...
**Repository**: `step-security/claude-code-action` — a secure drop-in replacement for `anthropics/claude-code-action`.
...
`base-action/` is also published standalone as `step-security/claude-code-base-action`. Don't break its public API. It reads config from `INPUT_`-prefixed env vars (set by `action.yml`), not from action inputs directly.
